...
$sql = "insert into add_blog (id,photo,blog_title,blog_detail,person,date,datatime) VALUES (id,'$photo','$blog_title','$blog_detail','$person','$date','$datetime')";

if ($conn->query($sql) === TRUE) {
header("location:./add_blog.php?msg=success");
} else {
header("location:./add_blog.php?msg=error");
}
...
<div class="main-container min-h-screen text-black dark:text-white-dark" :class="[$store.app.navbar]">
	...
	<?php include("./include/header.php")?>
		<div class="animate__animated p-6" :class="[$store.app.animation]">
			...
			<div x-data="sales">
				<div class="pt-5">
					<form class="space-y-5" action="./add_blog.php" method="post" enctype="multipart/form-data">
					  <div>
							<span class="text-black-dark text-[110px] inline-block mb-3">Enter Blog Photo</span>
							<input required type="file" name ="photo" placeholder="Enter Blog Photo Link" class="form-input" />
						</div>
						<div>
							<span class="text-black-dark text-[110px] inline-block mb-3">Enter Blog Title</span>
							<input type="text" name ="blog_title" placeholder="Enter Blog Title" class="form-input" />
						</div>
						<div>
							<span class="text-black-dark text-[110px] inline-block mb-3">Enter Blog Detail</span>
							<input type="text" name ="blog_detail" placeholder="Enter Blog Detail" class="form-input" />
						</div>
						<div>
							<span class="text-black-dark text-[110px] inline-block mb-3">Enter Author</span>
							<input type="text" name ="person" placeholder="Enter Author" class="form-input" />
						</div>
